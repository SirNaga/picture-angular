...
export class PostPageComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<Post[]>('http://localhost:9090/post').subscribe(data => {
      this.posts = data;
    });
  ...
  onSubmit() {
    ...
    if (this.createPostForm.valid) {
      ...
      this.restService.uploadImage(formData).subscribe(response => {
        ...
        const responsePost = this.restService.savePost(this.createPostForm.value);
        responsePost.subscribe(post =>{
